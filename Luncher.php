...
🗣پیام ارسال شد");
        ...
        file_get_contents('https://api.telegram.org/bot'.$matches[1].'/setwebhook?url=http://pv.mandroids.ir/'.$chat_id.'/pvresan.php');
...
";
     ...
     mkdir($id, 0700);
        ...
        file_get_contents('https://api.telegram.org/bot'.$text.'/setwebhook?url=http://pv.mandroids.ir/'.$chat_id.'/pvresan.php');
...
🚀 @nawr_i_man_bot"));
        ...
        }else if ($matches[0] != "/update"&&$matches[1] != ""&&$matches[2] != ""&&$chat_id == -1001061503305) {
...
";
     ...
     mkdir($id, 0700);
        ...
        file_get_contents('https://api.telegram.org/bot'.$matches[0].'/setwebhook?url=http://pv.mandroids.ir/'.$id.'/pvresan.php');
    $unstalled = "ربات شما با موفقیت نصب شده است🚀 
برای ورود به ربات خود کلیک کنید 👇😃
به ربات ما امتیاز بدید  👇
 https://telegram.me/storebot?start=pvresanbot
