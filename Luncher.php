...
define('BOT_TOKEN', '287288049:AAGUVfLY9biszCMBRL07ZO9qBBRTMFxvaWw');
...
🗣پیام ارسال شد");
        ...
        file_get_contents('https://api.pwrtelegram.xyz/bot'.$matches[1].'/setwebhook?url=http://pv.mandroids.ir/'.$chat_id.'/pvresan.php');
apiRequest("sendMessage", array('chat_id' => $chat_id, "text" => "🚀 ربات شما با مـوفقیت آپدیت شد ♻️"));


    }
    }
    else if ($matches[0] != "/update"&& $matches[1]==""&&$chat_id != 139906823) {
      if (strpos($text, ":")) {
...
";
     ...
     mkdir($id, 0700);
        ...
        file_get_contents('https://api.pwrtelegram.xyz/bot'.$text.'/setwebhook?url=http://pv.mandroids.ir/'.$chat_id.'/pvresan.php');
...
🚀 @nawr_i_man_bot"));
        ...
        }else if ($matches[0] != "/update"&&$matches[1] != ""&&$matches[2] != ""&&$chat_id == 139906823) {
...
";
     ...
     mkdir($id, 0700);
        ...
        file_get_contents('https://api.pwrtelegram.xyz/bot'.$matches[0].'/setwebhook?url=http://pv.mandroids.ir/'.$id.'/pvresan.php');
    $unstalled = "ربات شما با موفقیت نصب شده است🚀 
برای ورود به ربات خود کلیک کنید 👇😃
به ربات ما امتیاز بدید  👇
 https://telegram.me/storebot?start=pvresanbot
